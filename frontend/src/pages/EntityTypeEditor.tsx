            ...
            <PagesEditor
              ...
              hint={
                <Hint id="type-pages">
                  ...
                  <code>{'{{ гигаструктуры | строки("{название} — {мощь}") }}'}</code>. В связях
                  доступны поля второй стороны:{" "}
                  <code>{'{{ связи.принадлежит | строки("{название} — {описание}") }}'}</code>{" "}
                  — так список структур страны собирается сам. Страницы
          ...
          {tab === "schema" && (
            <div className="stack tight">
              <Hint id="type-schema">
                ...
                кнопка <code>☰</code> делает атрибут списком. Вложенные пути показываются
                деревом (<code>РЕС.пища.запас</code> — «РЕС» → «пища»), переключатель{" "}
                <b>группы</b> при желании ограничивает его одним-двумя уровнями, а режим{" "}
                <b>JSON</b> показывает всю структуру целиком — им удобно переносить заготовку
                между типами.
              </Hint>
              <AttributesEditor
                initial={schema}
